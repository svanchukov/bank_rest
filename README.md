...
Подключение к бд:
    url: jdbc:postgresql://localhost:5435/banks
    username: banks
    password: banks
...
5. После того нужно создать пользователя по примеру: { "email": "user@123.yandex.ru", "password": "123456" }
6. Такое же взаимодействие с токеном
7. Создание карты: { "ownerId" : id пользовател. }
8. Сделать перевод на карту с помощь метода админа, так как с баланс изначально 0, чтобы начать проверять нужно сделать пополнение карты извне, это может делать только админ 
9. После этого можно тестировать остальные сервисы админа, создание карт, проверка:
Карты:
...
POST	/admin/cards/create	Выпуск новой карты для любого пользователя.
POST	/admin/cards/{cardId}/block	Принудительная блокировка карты администратором.
POST	/admin/cards/{cardId}/activate	Разблокировка карты.
POST	/admin/cards/{cardId}/top-up	Админское пополнение баланса карты (имитация внесения наличных).
GET	/admin/cards	Просмотр всех существующих карт в базе данных.
